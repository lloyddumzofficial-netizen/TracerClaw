-- Repair the charged response from migrations 021/022. PostgreSQL's
-- timezone('utc', now()) returns timestamp without time zone, while the RPC
-- contract correctly declares attempt_created_at as timestamptz.
...
BEGIN
  ...
  RETURN QUERY SELECT 'charged'::text, inserted_attempt_id, 'processing'::text,
    NULL::text, NULL::text, now(), current_credits - requested_charge_amount;
END;
