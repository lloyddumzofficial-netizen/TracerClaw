...
export default function AdminTestEmailPage() {
  ...
  const [subject, setSubject] = useState("DesaynClaw Test Email");
  const [message, setMessage] = useState("Hello, this is a DesaynClaw test email.");
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const data = await safeJson(response, "Failed to send test email");
      setResult({
      ...
      setResult({ ok: false, message: error.message || "Failed to send test email." });
    } finally {
      setSending(false);
    }
  };
  ...
  if (loading) {
    return (
      <div className="start-screen-container" style={{ display: "flex", alignItems: "center", justifyContent: "center" }}>
        <div style={{ color: "#FFD700", fontSize: "14px", fontWeight: "500" }}>Loading Admin Email Test...</div>
      </div>
    );
  }
